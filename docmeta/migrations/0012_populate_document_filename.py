...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Make a document filename for each filename
        :param orm:
        :return:
        """
        for document in orm.Document.objects.all():
            for filename in document.filenames.all():
                orm.DocumentFileName(document=document, name=filename.name).save()

    def backwards(self, orm):
        raise Exception("Cannot back populate filenames")
